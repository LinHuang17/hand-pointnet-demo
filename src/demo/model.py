...
with addition of two-layer MLP after the final PCA layer.
"""

import torch
import torch.nn as nn
import math
from net_utils1 import group_points_2
# from net_utils2 import group_points_2
...
class PointNet_Plus(nn.Module):
    def __init__(self, opt):
        ...
        self.pca_size = opt.PCA_SZ
        self.output_size = opt.JOINT_NUM*opt.JOINT_DIM
        ...
        self.netR_FC = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            # B*512
            nn.Linear(nstates_plus_3[4], self.pca_size),
            # B*pca_size
        )

        self.netR_decoder = nn.Sequential(
            # B*pca_size -> B*50
            nn.Linear(self.pca_size, nstates_decoder[0]),
            nn.BatchNorm1d(nstates_decoder[0]),
            nn.ReLU(inplace=True),
            # B*50 -> B*(joint_num*joint_dim)
            nn.Linear(nstates_decoder[0], self.output_size),	
        )
    ...
    def forward(self, x, y):
        ...
        x = self.netR_FC(x)
        # B*pca_size
        x = self.netR_decoder(x)
        # B*(joint_num*joint_dim)
